...
const SingleUser = (props) =>{
    ...
    const removeBooking = async () => {
        try {
          let response = await fetch(
            `https://customhotels-494f8951a67d.herokuapp.com/users/${props.item._id}`,
            {
              method: "DELETE",
              headers: {
    ...
    return(
      ...
      >
      <div className="d-flex justify-content-between align-items-center"><div className="pl-4"><FaRegArrowAltCircleRight className="mb-1 mr-3" color="#E4D8E6"/></div><div className="eachBookingComponent" style={{fontSize:"17px"}}><b>{props.item.email}</b></div>{open === true ? <FaAngleDown className="pr-2" fontSize={30}/> : <FaAngleLeft className="pr-2" fontSize={30}/>} </div>
      </button>
      <Collapse in={open}>
        <Container>
            <Row>
